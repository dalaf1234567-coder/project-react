...
async function fastTranslate(text, src, tgt, signal) {
  const t = text.trim(); if (!t || src === tgt) return text;
  const key = `${src}|${tgt}|${t}`;
  ...
  const promise = (async () => {
    try {
      ...
      _tCache.set(key, result); return result;
    } catch {
      try {
...
select{background:#1c0505;color:#fde68a;border:1px solid #7f1d1d;border-radius:10px;padding:8px 10px;font-size:13px;outline:none;cursor:pointer}select:hover{border-color:#f59e0b}
...
.lrow{display:flex;align-items:center;gap:8px;margin-bottom:12px}.lrow span{font-size:12px;color:#b45309}
...
/* ════════════════════════════════════════════
   CARD STYLE (Seperti gambar yang dikirim user)
   ════════════════════════════════════════════ */
.card-container {
  display: flex;
...
function LearnPanel({ text, speechCode, flag }) {
  ...
  return (
    <div className="lpanel">
      ...
      <button className={`play-btn ${playing?"going":""}`} onClick={playAudio}><Volume2 size={15}/> {playing?"Playing...":"Play"}</button>
      <button className={`prac-btn ${pracRec?"going":""}`} onClick={pracRec?()=>pracRef.current?.stop():startPrac}>
        {pracRec?<><MicOff size={16}/>Stop</>:<><Mic size={16}/>Practice</>}
      </button>
      {score!==null&&(
        <div className="score-box">
          <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:11,color:"#7f3d3d"}}>Score</span><span style={{color:sc(score),fontWeight:700}}>{score}%</span></div>
          <div style={{height:6,background:"#2d0a0a",borderRadius:3,marginTop:4,overflow:"hidden"}}><div style={{height:6,background:sc(score),width:`${score}%`}}/></div>
        </div>
      )}
    </div>
  );
...
// ══════════════════════════════════════════════════════════════
//  CARD VIEW COMPONENT (Tampilan seperti screenshot)
// ══════════════════════════════════════════════════════════════
function TranslationCard({ original, translated, srcLang, tgtLang, onSpeak }) {
  return (
    <div className="card-container">
      {/* Card Asli */}
      <div className="card">
        <div className="card-header">
          <span className="card-lang-badge">{srcLang.flag} {srcLang.name}</span>
        </div>
        <div className="card-text">{original}</div>
      </div>

      {/* Card Terjemahan */}
      <div className="card" style={{borderColor: "rgba(16, 185, 129, 0.3)"}}>
        <div className="card-header">
          <span className="card-lang-badge" style={{color: "#6ee7b7", background: "rgba(16, 185, 129, 0.1)"}}>
            {tgtLang.flag} {tgtLang.name}
          </span>
        </div>
        <div className="card-text">{translated}</div>
        <div className="card-footer">
          <button className="card-play-btn" onClick={onSpeak}>
...
function ComicCanvas({ imgSrc, bubbles, pageNum }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      (bubbles || []).forEach(b => {
        const x = (b.x / 100) * img.naturalWidth; const y = (b.y / 100) * img.naturalHeight;
        const w = (b.w / 100) * img.naturalWidth; const h = (b.h / 100) * img.naturalHeight;
        if (w < 4 || h < 4) return;
        ...
        ctx.font = `700 ${fontSize}px 'Noto Sans',Arial`; ctx.fillStyle = "#0a0000"; ctx.textAlign = "center";
  ...
  const download = () => {
    const a = document.createElement("a"); a.download = `komik-${pageNum}.png`;
...
// ══════════════════════════════════════════════════════════════
//  DOCUMENT TAB (With Card View & Better OCR)
// ══════════════════════════════════════════════════════════════
function DocumentTab() {
  ...
  const MODES = {
    komik: { icon: "🎭", label: "Komik", hint: "Scan & overlay text", multi: true },
    buku: { icon: "📚", label: "Buku/Kartu", hint: "Tampilan Kartu (Card View)", multi: true },
  };

  useEffect(() => { resultsEnd.current?.scrollIntoView({ behavior: "smooth" }); }, [pages]);

  // -- Preprocessing: Fix Teks Putih & Kecil --
  const preprocessImage = (src) => {
    return new Promise(resolve => {
      const img = new Image(); img.crossOrigin = "Anonymous";
      img.onload = () => {
        const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d');
        // Perbesar 2x lipat untuk teks kecil (seperti "Steve Jobs")
        const scale = 2; 
        canvas.width = img.width * scale; canvas.height = img.height * scale;
        // Tingkatkan kontras & grayscale utk teks putih di bg gelap
  ...
  const processPage = async (id, imgSrc, tgt) => {
    ...
    try {
      ...
      // PSM 3: Fully automatic (bagus untuk buku), PSM 11 untuk komik sparse
      await worker.setParameters({ tessedit_pageseg_mode: mode === 'komik' ? '11' : '3' });
      
      const { data: { lines, text } } = await worker.recognize(processedImg);
      await worker.terminate();

      const validLines = lines.filter(l => l.text.trim().length > 1 && l.confidence > 45);

      if (mode === "komik") {
        const img = new Image(); img.src = imgSrc; await new Promise(r => img.onload = r);
        const W = img.naturalWidth; const H = img.naturalHeight;
        const bubbles = validLines.map(line => ({
          x: (line.bbox.x0 / W) * 100, y: (line.bbox.y0 / H) * 100,
          w: ((line.bbox.x1 - line.bbox.x0) / W) * 100, h: ((line.bbox.y1 - line.bbox.y0) / H) * 100,
          text: line.text.trim()
        ...
        // Mode Buku/Kartu
        const fullText = text; // Ambil text utuh, biarkan format alami
  ...
  return (
    <div className="doc-body">
      ...
      <div className="doc-lang-bar">
        <span style={{fontSize:11,color:"#b45309"}}>Target:</span>
      ...
      {!pages.length && (
        <div className="doc-upload-zone" onClick={() => fileRef.current?.click()}>
          ...
          <div style={{fontSize:12,color:"#b45309"}}>Gratis, otomatis perbaiki teks putih/kecil</div>
        </div>
      )}
          ...
          <div key={p.id} className={`doc-page ${p.status}`}>
            <div className="doc-page-hdr">
              ...
              <div className="doc-page-info">
                <div className="doc-page-num">Page {i + 1}</div>
                <div className={`doc-page-status ${p.status}`}>
                  {p.status === "ocr" && "🔍 Reading (Enhanced)..." }
                  {p.status === "translating" && "⚡ Translating..."}
                  {p.status === "done" && "✅ Done"}
                </div>
              </div>
            </div>
            ...
            {mode === "buku" && p.status === "done" && (
              <div style={{padding: "0 4px 4px"}}>
                <TranslationCard 
                   original={p.orig} 
                   translated={p.trans} 
                   srcLang={{flag: "🌐", name: "Original"}}
                   tgtLang={tgtLang}
                   onSpeak={() => {
                     if(window.speechSynthesis) {
                       window.speechSynthesis.cancel();
                       const u = new SpeechSynthesisUtterance(p.trans);
                       u.lang = tgtLang.speech;
                       window.speechSynthesis.speak(u);
                     }
                   }}
                />
...
// ── VideoTab & Main App (Sama seperti sebelumnya) ──
function VideoTab() {
  ...
  const stopAll = () => { streamRef.current?.getTracks().forEach(t => t.stop()); try { srRef.current?.stop(); } catch {} };
  const toggle = async () => {
    if (isLive) { setIsLive(false); isLiveRef.current = false; stopAll(); }
    else {
      isLiveRef.current = true; setIsLive(true);
      try { const stream = await navigator.mediaDevices.getUserMedia({ video: true }); streamRef.current = stream; if (videoRef.current) videoRef.current.srcObject = stream; } catch {}
      const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
      if (SR) { const r = new SR(); srRef.current = r; r.continuous = true; r.interimResults = true; r.lang = srcLang.speech;
        r.onresult = async (e) => { let txt = ""; for (let i = e.resultIndex; i < e.results.length; i++) txt += e.results[i][0].transcript; const tr = await fastTranslate(txt, srcLang.code, tgtLang.code); setSubtitle(tr); };
        r.start();
      }
    }
  };
  return (
    <div className="vfull">
      ...
      {!isLive && <div className="vfull-placeholder"><Video size={52}/><p style={{color:"#fca5a5"}}>Tap Start</p></div>}
      {isLive && subtitle && <div className="vfull-sub"><span className="vfull-sub-text">{subtitle}</span></div>}
      <div className="vfull-bottom">
        {!isLive && ( <div className="vlang-row-h"> <select value={srcLang.code} onChange={e=>setSrcLang(LANGS.find(l=>l.code===e.target.value))}>{LANGS.map(l=><option key={l.code} value={l.code}>{l.flag}</option>)}</select> <span>→</span> <select value={tgtLang.code} onChange={e=>setTgtLang(LANGS.find(l=>l.code===e.target.value))}>{LANGS.map(l=><option key={l.code} value={l.code}>{l.flag}</option>)</select> </div> )}
        <button className={`vstart-btn ${isLive?"live":"idle"}`} onClick={toggle}> {isLive ? "Stop" : "Start"} </button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [translating, setTranslating] = useState(false); const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [showPhoto, setShowPhoto] = useState(false); const [photoImg, setPhotoImg] = useState(null);
  const [photoTgt, setPhotoTgt] = useState(LANGS[1]); const [photoResult, setPhotoResult] = useState(null);
  const [error, setError] = useState(""); const [openLearn, setOpenLearn] = useState(null);
  ...
  const startListen=side=>{
    ...
    r.onerror=()=>setListening(null); r.start();
  };
  ...
  const handlePhoto = async e => {
    ...
    reader.onload = async ev => {
      const url = ev.target.result; setPhotoImg(url); setPhotoResult({original:"Processing...",translated:""});
      try {
        if (!window.Tesseract) await loadTesseract();
        const img = new Image(); img.src = url; await new Promise(r => img.onload = r);
        const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d');
        canvas.width = img.width * 2; canvas.height = img.height * 2;
        ctx.filter = 'contrast(1.5) grayscale(1)'; ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        const processed = canvas.toDataURL('image/png');
        const worker = await window.Tesseract.createWorker('eng+ind', 1);
        const { data: { text } } = await worker.recognize(processed); await worker.terminate();
      ...
      } catch (err) { setPhotoResult({ original: "Failed", translated: "" }); }
    };
    reader.readAsDataURL(file);
  };
      ...
      <div className="app">
          ...
          <div className="feed">
            {messages.map(m=>(
              <div key={m.id} className={`bwrap ${m.side}`}>
                ...
                <button className={`learn-btn ${m.side} ${openLearn===m.id?"open":""}`} onClick={()=>setOpenLearn(openLearn===m.id?null:m.id)}>Learn {openLearn===m.id?<ChevronUp/>:<ChevronDown/>}</button>
        ...
        {showPhoto&&<div className="pov">
          ...
          :<div>
            ...
            {photoResult&&<div style={{marginTop:10}}>
               <TranslationCard 
                  original={photoResult.original} 
                  translated={photoResult.translated}
                  srcLang={{flag: "🌐", name: "Original"}}
                  tgtLang={photoTgt}
                  onSpeak={()=>speak(photoResult.translated, photoTgt.speech)}
               />
            </div>}
