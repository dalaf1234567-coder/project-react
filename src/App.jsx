...
// ══════════════════════════════════════════════════════════════
//  CSS STYLES (Sama seperti sebelumnya, dipendekkan di sini)
// ══════════════════════════════════════════════════════════════
const css = `
...
function LearnPanel({ text, speechCode, flag }) {
  ...
  return (
    <div className="lpanel">
      ...
      <button className={`play-btn ${playing?"going":""}`} onClick={playAudio}><Volume2 size={15}/>{playing?"Sedang diputar...":"Dengarkan"}</button>
      <button className={`prac-btn ${pracRec?"going":""}`} onClick={pracRec?()=>pracRef.current?.stop():startPrac}>
        {pracRec?<><MicOff size={16}/>Stop</>:<><Mic size={16}/>Praktik Ucap</>}
      </button>
      {score!==null&&(
        <div className="score-box">
          <div className="score-row"><span className="score-lbl">Skor</span><span className="score-num" style={{color:sc(score)}}>{score}%</span></div>
          ...
          {heard&&<div className="heard">Terdengar: <span>"{heard}"</span></div>}
        </div>
      )}
    </div>
  );
...
// ══════════════════════════════════════════════════════════════
//  DOCUMENT TAB (FIX: Preprocessing & Better Grouping)
// ══════════════════════════════════════════════════════════════
function DocumentTab() {
  ...
  const MODES = {
    komik: { icon: "🎭", label: "Komik", hint: "Scan teks di dalam bubble (Gratis)", multi: true },
    buku: { icon: "📚", label: "Buku", hint: "Scan halaman buku/dokumen", multi: true },
  ...
  // -- FIX 1: Preprocessing Image (Perbaiki Teks Putih & Kecil) --
  const preprocessImage = (src) => {
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        // 1. Perbesar gambar (Upscale) untuk teks kecil
        const scale = img.width < 800 ? 2 : 1.5; 
        canvas.width = img.width * scale;
        canvas.height = img.height * scale;
        
        // 2. Terapkan Filter Kontras & Grayscale
        // Ini membantu Tesseract membaca teks putih di background gelap
        // Dengan mengubah kontras tinggi, teks putih akan jadi hitam pekat di putih
        ctx.filter = 'contrast(1.75) grayscale(1) brightness(1.1)';
  ...
  const processPage = async (id, imgSrc, tgt) => {
    ...
    try {
      ...
      // 1. Perbaiki gambar dulu
      const processedImg = await preprocessImage(imgSrc);

      // 2. Jalankan Tesseract dengan konfigurasi lebih baik
      const worker = await window.Tesseract.createWorker('eng+ind', 1, {
        logger: m => console.log(m.status, m.progress)
      });

      // PSM 11: Sparse text (bagus untuk komik/gambar dengan teks menyebar)
      // PSM 3: Default (bagus untuk buku)
      ...
      // 3. Filter hasil yang kosong/noise
      const validLines = lines.filter(l => l.text.trim().length > 1 && l.confidence > 40);
      ...
      if (mode === "komik") {
        ...
        // Mapping lines ke bubbles
        const bubbles = validLines.map(line => ({
          x: (line.bbox.x0 / W) * 100,
          y: (line.bbox.y0 / H) * 100,
          w: ((line.bbox.x1 - line.bbox.x0) / W) * 100,
          h: ((line.bbox.y1 - line.bbox.y0) / H) * 100,
          text: line.text.trim()
    ...
    <div className="doc-body">
      ...
      {!pages.length && (
        <div className="doc-upload-zone" onClick={() => fileRef.current?.click()}>
          ...
          <div style={{fontSize:11,color:"#5a2020",display:"flex",alignItems:"center",gap:4}}><Zap size={11}/> Gratis & Cerdas (Auto-Contrast)</div>
        </div>
      )}
          ...
          <div key={p.id} className={`doc-page ${p.status}`}>
            ...
            {mode === "komik" && p.status === "done" && (
              p.bubbles && p.bubbles.length > 0
                ...
                : <div className="comic-no-bubble">Tidak ada teks terdeteksi (coba gambar yang lebih jelas)</div>
            )}
...
export default function App() {
  ...
  // Handle Photo with Preprocessing
  const handlePhoto = async e => {
    ...
    reader.onload = async ev => {
      ...
      try {
        ...
        setPhotoResult({ original: "Gagal scan", translated: "" });
      }
    };
    reader.readAsDataURL(file);
  };
          ...
          <div className="feed">
            {messages.map(m=>(
              <div key={m.id} className={`bwrap ${m.side}`}>
                ...
                <button className={`learn-btn ${m.side} ${openLearn===m.id?"open":""}`} onClick={()=>setOpenLearn(openLearn===m.id?null:m.id)}>Pelajari {openLearn===m.id?<ChevronUp/>:<ChevronDown/>}</button>
